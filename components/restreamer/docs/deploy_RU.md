...
### 1.1. Выберите Ubuntu 20.10 образ

![Ubuntu 20.10 образ](images/do1.jpg)

> __ВНИМАНИЕ__: Другие образы не поддерживаются! НЕ выбирайте их!


### 1.2. Выбите желаемую конфигурацию сервера
...
[DigitalOcean] требует, чтобы созданный сервер был доступен через [SSH].
...
После запуска процесса создания дроплета подождите от 5 до 15 минут пока сервер настроится и Ephyr re-streamer установится.
